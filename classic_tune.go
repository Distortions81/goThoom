...
// Classic CLTF scheduling notes
//
// Historical bug we fixed:
// - Chord events [ ... ] were incorrectly advancing the timeline by their
//   duration. In the classic client (CTuneBuilder), chords do NOT advance time;
//   only melody notes and rests (p) advance the beat counter. This led to
//   songs sounding 2–4x slower when written in the common "chord + short rest"
//   idiom like: p@70([g]8 p1 [d]8 p1 ...). The correct behavior schedules the
//   chords at the current melody time and advances time by the following rest,
//   producing tightly spaced hits (≈1s for four chord strikes at 70 BPM).
//
// Emulation details:
...
func classicNotesFromTune(tune string, inst instrument, tempo int, velocity int) []Note {
    ...
    for i < len(s) {
        ...
        if c == '@' {
            i++
            sign := byte(0)
            if i < len(s) && (s[i] == '+' || s[i] == '-' || s[i] == '=') { sign = s[i]; i++ }
            val := 0
            for i < len(s) && s[i] >= '0' && s[i] <= '9' { val = val*10 + int(s[i]-'0'); i++ }
            nt := tempo
            if val == 0 && sign == 0 {
                nt = 120
            } else {
        ...
        if c == '%' || c == '{' || c == '}' {
            i++
            d := 0
            if i < len(s) && s[i] >= '1' && s[i] <= '9' { d = int(s[i]-'0'); i++ }
            switch c {
            case '%': if d == 0 { volMel10 = 10 } else { volMel10 = d }
            case '{': if d == 0 { volMel10 -= 1 } else { volMel10 -= d }
            case '}': if d == 0 { volMel10 += 1 } else { volMel10 += d }
            }
            if volMel10 < 0 { volMel10 = 0 }
            if volMel10 > 10 { volMel10 = 10 }
            continue
        }
        // rest
        if c == 'p' {
        ...
        if c == '[' {
            ...
            for i < len(s) && s[i] != ']' {
                ...
                if s[i] == '%' || s[i] == '{' || s[i] == '}' {
                    ...
                    if i < len(s) && s[i] >= '1' && s[i] <= '9' { d = int(s[i]-'0'); i++ }
                    switch tok {
                    case '%': if d == 0 { volCh10 = 10 } else { volCh10 = d }
                    case '{': if d == 0 { volCh10 -= 1 } else { volCh10 -= d }
                    case '}': if d == 0 { volCh10 += 1 } else { volCh10 += d }
                    }
                    if volCh10 < 0 { volCh10 = 0 }
                    if volCh10 > 10 { volCh10 = 10 }
                    continue
                }
                if isNoteLetter(s[i]) {
                    key, _ := parseNotePitch(s, &i, innerOct)
                    if key >= 0 { ks = append(ks, key) }
                    continue
                }
            ...
            if len(ks) > 0 {
                ...
                if len(activeLong) > 0 {
                    for _, idx := range activeLong {
                        ...
                        if end < int(notes[idx].Start.Milliseconds()) { end = int(notes[idx].Start.Milliseconds()) }
                ...
                for _, k := range ks {
                    if strictCLTF {
                        ...
                        if !allowedNoteForInst(inst, key) { continue }
                        // reassign key after filter
                        if remaining > 0 && count >= remaining { break }
        ...
        if isNoteLetter(c) {
            ...
            b := 2.0
            if unicode.IsUpper(rune(c)) { b = 4.0 }
            // optional duration override
            if i < len(s) && s[i] >= '1' && s[i] <= '9' { b = float64(s[i]-'0'); i++ }
    ...
    if curMelTicks == 0 {
        ...
        if len(chordIdx) == 0 { return notes }
        keep := make([]bool, len(notes))
        for i := range keep { keep[i] = true }
        for _, idx := range chordIdx { if idx >= 0 && idx < len(keep) { keep[idx] = false } }
        out := make([]Note, 0, len(notes)-len(chordIdx))
        for i, n := range notes { if keep[i] { out = append(out, n) } }
        return out
    }
    // Otherwise clip chord notes that exceed melody end
    for _, idx := range chordIdx {
        if idx < 0 || idx >= len(notes) { continue }
...
func parseNotePitch(s string, i *int, octave int) (int, bool) {
    ...
    if *i >= len(s) { return -1, false }
    c := rune(s[*i])
    off := noteOffsetClassic(c)
    if off < 0 { return -1, false }
    *i = *i + 1
...
func expandLoopsClassic(s string) string {
    ...
    for i := 0; i < len(s); {
        if s[i] != '(' { out.WriteByte(s[i]); i++; continue }
        ...
        count := 1
        if i < len(s) && s[i] >= '1' && s[i] <= '9' { count = int(s[i]-'0'); i++ }
...
func splitEndings(content string) (main string, endings map[int]string, def string) {
    ...
    type seg struct{ idx int; kind byte; label int }
    splits := []seg{}
    depth := 0
    ...
    for si := 0; si < len(splits); si++ {
        ...
        if splits[si].kind == '|' { start += 2 } else { start += 1 }
        end := len(content)
        if si+1 < len(splits) { end = splits[si+1].idx }
