...
// MeasureTextWidth safely measures the width of s using face. If the face lacks a
...
func MeasureTextWidth(s string, face text.Face) float64 {
	if gf, ok := face.(*text.GoTextFace); ok {
		if gf.Source == nil {
			...
			return float64(len([]rune(s))) * (gf.Size * 0.6)
		}
	}
...
func WrapText(s string, face text.Face, maxWidth float64) (int, []string) {
	...
	for _, para := range strings.Split(s, "\n") {
		...
		for _, tok := range tokens {
			...
			for _, r := range tok {
				rw := MeasureTextWidth(string(r), face)
