...
[[ "$(go list -m -f '{{.Version}}' github.com/hajimehoshi/ebiten/v2)" = v2.10.1-0.20260909104753-f3e017a2e5c2 ]] || {
    echo 'The diagnostic patch requires Ebitengine commit f3e017a2e5c2.' >&2
    exit 1
}
...
git -C "$probe_output/ebiten" apply "$probe_root/build-scripts/diagnostics/ebitengine-f3e017a2e5c2-gpu-timing.patch"
...
CGO_ENABLED=0 go test -modfile="$probe_output/probe.mod" -c -o "$probe_output/render.test" .
